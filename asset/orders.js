function sectionChange(a, b){
    const btn = document.querySelectorAll(".nav-item-link");
    const selectedbtn = document.querySelector(`.${a}`);
    ...
    const selectedSections = document.querySelector(`#${b}`);

    sections.forEach((button) => {
      button.classList.remove("active");
      selectedSections.classList.add("active");
    });
}
...
const mainProducts = [
    ...
    },
    {
        ...
        description: 'Create stunning user interfaces and seamless experiences. Learn design principles, prototyping with Figma, user research, and visual communication for digital products.',
        price: 30000,
        originalPrice: 35000,
        category: 'UI/UX',
...
function loadCart() {
    try {
        ...
        if (saved) mainCart = JSON.parse(saved);
    } catch (e) {
        mainCart = [];
    }
}
...
function saveCart() {
    localStorage.setItem('techstars_cart', JSON.stringify(mainCart));
}
...
function addToCart(productId) {
    ...
    const existing = mainCart.find(item => item.id === productId);
    if (existing) {
        existing.quantity += 1;
    } else {
        mainCart.push({
...
function removeFromCart(productId) {
    cart = mainCart.filter(item => item.id !== productId);
    saveCart();
    updateCartUI();
    renderProducts();
...
function updateQuantity(productId, delta) {
    const item = mainCart.find(item => item.id === productId);
    if (!item) return;
...
function getCartTotal() {
    return mainCart.reduce((sum, item) => sum + (item.price * item.quantity), 0);
}

function getCartCount() {
    return mainCart.reduce((sum, item) => sum + item.quantity, 0);
}

function getBundleDiscount() {
    const count = mainCart.reduce((sum, item) => sum + item.quantity, 0);
...
function updateCartUI() {
    ...
    if (count === 0) {
        ...
        if (cartItemsEl) {
            ...
            cartItemsEl.innerHTML = mainCart.map(item => `
                <div class="cart-item">
...
function renderProducts() {
    ...
    if (resultsCount) {
        if (filtered.length === mainProducts.length) {
            resultsCount.textContent = `Showing all ${mainProducts.length} courses`;
        } else {
            resultsCount.textContent = `Showing ${filtered.length} of ${mainProducts.length} courses`;
        }
    }
    ...
    grid.innerHTML = filtered.map((product, index) => {
        const inCart = mainCart.some(item => item.id === product.id);
